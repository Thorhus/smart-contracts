...
library Call {
    ...
    function loadAddress(bytes memory sig, uint idx) public pure returns (address) {
        address influencer;
        idx += 20;
        assembly
        {
    ...
    function loadUint8(bytes memory sig, uint idx) public pure returns (uint8) {
        uint8 weight;
        idx += 1;
        assembly
        {
    ...
    function recoverSigMemory(bytes memory sig) private pure returns (address[] memory, address[] memory, uint8[] memory, uint[] memory, uint) {
    ...
    function recoverSigParts(bytes memory sig, address last_address) private pure returns (address[] memory, address[] memory, uint8[] memory, uint[] memory) {
        // sig structure:
    ...
    function recoverSig(bytes memory sig, address old_key, address last_address) public pure returns (address[] memory, address[] memory, uint8[] memory) {
        // validate sig AND
